...
public interface FriendRepository extends JpaRepository<FriendEntity, FriendId> {
  ...
  @Query(
      "SELECT f FROM FriendEntity f WHERE f.user = :user AND status = 'accepted' AND f.friend.id NOT IN "
  ...
  Page<FriendEntity> findFriendsExcludingBlocked(
      @Param("user") Long user, @Param("currentUser") Long currentUser, Pageable pageable);

  @Query(
      "SELECT CASE WHEN COUNT(f) > 0 THEN true ELSE false END FROM FriendEntity f WHERE f.friend.id = :user AND f.user = :currentUser AND status = 'accepted'")
