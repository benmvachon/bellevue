...
public interface ReviewRepository extends JpaRepository<ReviewEntity, Long> {
    ...
    Page<ReviewEntity> findByRecipeId(@Param("recipe") Long recipe, @Param("user") Long user, Pageable pageable);
    ...
    @Query("SELECT DISTINCT r FROM ReviewEntity r "
            ...
            + "ORDER BY r.updated DESC")
    Page<ReviewEntity> findByAuthorId(@Param("author") Long author, @Param("user") Long user, Pageable pageable);
    ...
    @Query("SELECT CASE WHEN COUNT(r) > 0 THEN true ELSE false END FROM ReviewEntity r WHERE r.id = :review AND r.author.id = :user")
