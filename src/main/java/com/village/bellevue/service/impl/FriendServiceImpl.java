...
public class FriendServiceImpl implements FriendService {
    ...
    public void request(Long user) throws FriendshipException {
        ...
        try (Connection connection = dataSource.getConnection(); CallableStatement stmt = connection.prepareCall("{call request_friend(?, ?)}")) {
            ...
            throw new FriendshipException("Failed to request friendship. SQL command error: " + e.getMessage(), e);
        } finally {
            entityManager.flush();
    ...
    public Optional<ScrubbedUserEntity> read(Long user) throws FriendshipException {
        if (getAuthenticatedUserId().equals(user)) {
            return Optional.of(userRepository.findById(user).map(ScrubbedUserEntity::new).orElseThrow(() -> new FriendshipException("User not found with id: " + user)));
        }
        if (isBlockedBy(user)) {
            return Optional.empty();
        }
        Optional<FriendEntity> friend = friendRepository.findById(new FriendId(getAuthenticatedUserId(), user));
        if (friend.isPresent()) {
    ...
    @Cacheable(value = FRIENDSHIP_STATUS_CACHE_NAME, key = "T(com.village.bellevue.config.CacheConfig).getCacheKey(T(com.village.bellevue.service.impl.FriendServiceImpl).STATUS_CACHE_KEY, T(com.village.bellevue.config.security.SecurityConfig).getAuthenticatedUserId(), #user)")
    @Override
    ...
    @Cacheable(value = FRIENDSHIP_STATUS_CACHE_NAME, key = "T(com.village.bellevue.config.CacheConfig).getCacheKey(T(com.village.bellevue.service.impl.FriendServiceImpl).IS_FRIEND_CACHE_KEY, T(com.village.bellevue.config.security.SecurityConfig).getAuthenticatedUserId(), #user)")
    @Override
    ...
    @Cacheable(value = FRIENDSHIP_STATUS_CACHE_NAME, key = "T(com.village.bellevue.config.CacheConfig).getCacheKey(T(com.village.bellevue.service.impl.FriendServiceImpl).IS_BLOCKING_CACHE_KEY, T(com.village.bellevue.config.security.SecurityConfig).getAuthenticatedUserId(), user)")
    @Override
    ...
    public Page<FriendEntity> readAll(Long user, int page, int size) throws FriendshipException {
        return friendRepository.findFriendsExcludingBlocked(user, getAuthenticatedUserId(), PageRequest.of(page, size));
    }
    ...
    private void evictCaches(Long user) {
        ...
        evictKeysByPattern(redisTemplate, RECIPE_SECURITY_CACHE_NAME, getUserCacheKeyPattern(RecipeServiceImpl.CAN_READ_CACHE_KEY, currentUser));
        evictKeysByPattern(redisTemplate, RECIPE_SECURITY_CACHE_NAME, getUserCacheKeyPattern(RecipeServiceImpl.CAN_READ_CACHE_KEY, user));
        evictKeysByPattern(redisTemplate, RECIPE_SECURITY_CACHE_NAME, getUserCacheKeyPattern(RecipeServiceImpl.CAN_UPDATE_CACHE_KEY, currentUser));
        evictKeysByPattern(redisTemplate, RECIPE_SECURITY_CACHE_NAME, getUserCacheKeyPattern(RecipeServiceImpl.CAN_UPDATE_CACHE_KEY, user));

        evictKeysByPattern(redisTemplate, REVIEW_SECURITY_CACHE_NAME, getUserCacheKeyPattern(ReviewServiceImpl.CAN_READ_CACHE_KEY, currentUser));
        evictKeysByPattern(redisTemplate, REVIEW_SECURITY_CACHE_NAME, getUserCacheKeyPattern(ReviewServiceImpl.CAN_READ_CACHE_KEY, user));
        evictKeysByPattern(redisTemplate, REVIEW_SECURITY_CACHE_NAME, getUserCacheKeyPattern(ReviewServiceImpl.CAN_UPDATE_CACHE_KEY, currentUser));
        evictKeysByPattern(redisTemplate, REVIEW_SECURITY_CACHE_NAME, getUserCacheKeyPattern(ReviewServiceImpl.CAN_UPDATE_CACHE_KEY, user));
    }
}
