...
public class RecipeServiceImpl implements RecipeService {
    ...
    public Optional<RecipeModel> read(Long id) throws AuthorizationException, RecipeException {
        if (canRead(id)) {
            ...
            if (recipe.isPresent()) {
                ...
                return Optional.of(new RecipeModel(recipeEntity, ratingRepository.findById(new AggregateRatingId(getAuthenticatedUserId(), id))));
            }
            return Optional.empty();
        }
        throw new AuthorizationException("Currently authenticated user is not authorized to read recipe");
    }
    ...
    @Override
    public Page<SimpleRecipeEntity> readAll(Long author, int page, int size) throws AuthorizationException, RecipeException {
        try {
            ...
            if (status.isEmpty() || !FriendshipStatus.ACCEPTED.equals(status.get())) {
                throw new AuthorizationException("Currently authenticated user is not authorized to read recipes");
            }
    ...
    @Transactional
    @Override
    public RecipeModel update(Long id, RecipeEntity updatedRecipe) throws AuthorizationException, RecipeException {
        if (!canUpdate(id)) {
            throw new AuthorizationException("Currently authenticated user is not authorized to update recipe: " + id);
        }
        // ensure that the correct recipe is being updated
        updatedRecipe.setId(id);

        return new RecipeModel(save(updatedRecipe), ratingRepository.findById(new AggregateRatingId(getAuthenticatedUserId(), id)));
    }
    ...
    public Long cook(Long recipe) throws AuthorizationException, RecipeException {
        if (!canRead(recipe)) {
            throw new AuthorizationException("Currently authenticated user is not authorized to review recipe");
        }
        try (Connection connection = dataSource.getConnection(); CallableStatement stmt = connection.prepareCall("{call start_cooking(?, ?, ?)}")) {
    ...
    public void delete(Long id) throws AuthorizationException, RecipeException {
        if (!canUpdate(id)) {
            throw new AuthorizationException("Currently authenticated user is not authorized to delete recipe: " + id);
        }
        try {
    ...
    public Page<SimpleRecipeEntity> search(
        ...
        return recipeRepository.findByNameStartingWithIgnoreCase(query, getAuthenticatedUserId(), ingredients, skills, equipment, PageRequest.of(page, size));
    }

    @Cacheable(value = RECIPE_SECURITY_CACHE_NAME, key = "T(com.village.bellevue.config.CacheConfig).getCacheKey(T(com.village.bellevue.service.impl.RecipeServiceImpl).CAN_READ_CACHE_KEY, T(com.village.bellevue.config.security.SecurityConfig).getAuthenticatedUserId(), #id)")
    @Override
    public boolean canRead(Long id) {
        return recipeRepository.canRead(id, getAuthenticatedUserId());
    }

    @Cacheable(value = RECIPE_SECURITY_CACHE_NAME, key = "T(com.village.bellevue.config.CacheConfig).getCacheKey(T(com.village.bellevue.service.impl.RecipeServiceImpl).CAN_UPDATE_CACHE_KEY, T(com.village.bellevue.config.security.SecurityConfig).getAuthenticatedUserId(), #id)")
    @Override
    ...
    private void evictRecipeSecurityCaches(Long id) {
        evictKeysByPattern(redisTemplate, RECIPE_SECURITY_CACHE_NAME, getEntityCacheKeyPattern(CAN_READ_CACHE_KEY, id));
        evictKeysByPattern(redisTemplate, RECIPE_SECURITY_CACHE_NAME, getEntityCacheKeyPattern(CAN_UPDATE_CACHE_KEY, id));
    }
    ...
    private RecipeEntity bindRelatedEntities(RecipeEntity recipeEntity) {
        ...
        if (recipeEntity.getIngredients() != null) {
            recipeEntity.getIngredients().forEach((ingredient) -> {
                IngredientEntity managedIngredient = ingredientRepository.findById(ingredient.getIngredient().getId()).get();
                ingredient.setIngredient(managedIngredient);
                ingredient.setRecipe(recipeEntity.getId());
            });
        }
        if (recipeEntity.getSkills() != null) {
            recipeEntity.getSkills().forEach((skill) -> {
        ...
        if (recipeEntity.getEquipment() != null) {
            recipeEntity.getEquipment().forEach((equipmentEntity) -> {
                EquipmentEntity managedEquipment = equipmentRepository.findById(equipmentEntity.getEquipment().getId()).get();
