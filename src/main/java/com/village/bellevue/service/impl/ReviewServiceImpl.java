...
public class ReviewServiceImpl implements ReviewService {
    ...
    public ReviewServiceImpl(ReviewRepository reviewRepository, RecipeService recipeService, FriendService friendService, RedisTemplate<String, Object> redisTemplate, DataSource dataSource) {
    ...
    public Long create(ReviewEntity review) throws AuthorizationException, ReviewException {
        ...
        try (Connection connection = dataSource.getConnection(); CallableStatement stmt = connection.prepareCall("{call add_review(?, ?, ?, ?, ?)}")) {
    ...
    public Optional<ReviewEntity> read(Long id) throws AuthorizationException, ReviewException {
        if (!canUpdate(id) && !canRead(id)) {
            throw new AuthorizationException("Currently authenticated user is not friends with review's author");
        }
    ...
    @Override
    public Page<ReviewEntity> readAllByRecipe(Long recipe, int page, int size) throws AuthorizationException, ReviewException {
        if (!recipeService.canRead(recipe)) {
            throw new AuthorizationException("Currently authenticated user is not authorized to review recipe");
        }
        return reviewRepository.findByRecipeId(recipe, getAuthenticatedUserId(), PageRequest.of(page, size));
    }

    @Override
    public Page<ReviewEntity> readAllByAuthor(Long author, int page, int size) throws AuthorizationException, ReviewException {
        Long user = getAuthenticatedUserId();
        try {
            if (!user.equals(author) && !friendService.isFriend(author)) {
                throw new AuthorizationException("Currently authenticated user is not authorized to read reviews");
            }
    ...
    @Override
    @Transactional
    public ReviewEntity update(Long id, ReviewEntity updatedReview) throws AuthorizationException, ReviewException {
        if (!canUpdate(id)) {
            throw new AuthorizationException("Currently authenticated user did not author review: " + id);
        }
        try (Connection connection = dataSource.getConnection(); CallableStatement stmt = connection.prepareCall("{call update_review(?, ?, ?)}")) {
            stmt.setLong(1, id);
    ...
    public void delete(Long id) throws AuthorizationException, ReviewException {
        ...
        try (Connection connection = dataSource.getConnection(); CallableStatement stmt = connection.prepareCall("{call delete_review(?)}")) {
            stmt.setLong(1, id);
            stmt.executeUpdate();
        } catch (SQLException e) {
    ...
    @Cacheable(value = REVIEW_SECURITY_CACHE_NAME, key = "T(com.village.bellevue.config.CacheConfig).getCacheKey(T(com.village.bellevue.service.impl.ReviewServiceImpl).CAN_READ_CACHE_KEY, T(com.village.bellevue.config.security.SecurityConfig).getAuthenticatedUserId(), #id)")
    @Override
    public boolean canRead(Long id) {
        return reviewRepository.canRead(id, getAuthenticatedUserId());
    }

    @Cacheable(value = REVIEW_SECURITY_CACHE_NAME, key = "T(com.village.bellevue.config.CacheConfig).getCacheKey(T(com.village.bellevue.service.impl.ReviewServiceImpl).CAN_UPDATE_CACHE_KEY, T(com.village.bellevue.config.security.SecurityConfig).getAuthenticatedUserId(), #id)")
    @Override
    ...
    private void evictCaches(Long id) {
        evictKeysByPattern(redisTemplate, REVIEW_SECURITY_CACHE_NAME, getEntityCacheKeyPattern(CAN_READ_CACHE_KEY, id));
        evictKeysByPattern(redisTemplate, REVIEW_SECURITY_CACHE_NAME, getEntityCacheKeyPattern(CAN_UPDATE_CACHE_KEY, id));
    }
}
