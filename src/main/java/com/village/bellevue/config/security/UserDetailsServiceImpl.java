...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public void delete() throws AuthorizationException {
        ...
        try (Connection connection = dataSource.getConnection(); CallableStatement stmt = connection.prepareCall("{call delete_user(?)}")) {
            stmt.setLong(1, user);
            stmt.executeUpdate();
        } catch (SQLException e) {
            throw new AuthorizationException("Failed to delete user. SQL command error: " + e.getMessage(), e);
        }
    }
