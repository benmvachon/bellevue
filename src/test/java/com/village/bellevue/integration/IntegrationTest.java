...
public class IntegrationTest extends IntegrationTestWrapper {
    ...
    void testCreateUser() {
        ...
        ResponseEntity<ScrubbedUserEntity> response = restTemplate
            .postForEntity("http://localhost:" + port + "/api/user/signup", newUser, ScrubbedUserEntity.class);
    ...
    void acceptFriends() throws FriendshipException {
        ...
        acceptAs("liam_wil", newUser.getId(), false); // id 9 was not requested, he should not be able to accept
        login("liam_wil");
        ResponseEntity<ScrubbedUserEntity> response = restTemplate
            .getForEntity("http://localhost:" + port + "/api/friend/" + newUser.getId(), ScrubbedUserEntity.class);
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND); // even after "accepting" the new user, they should not be visible to liam_wil
        logout();
        ...
        response = restTemplate
            .getForEntity("http://localhost:" + port + "/api/friend/" + newUser.getId(), ScrubbedUserEntity.class);
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND); // even after "requesting" the new user, they should not be visible to liv_w
        logout();
    ...
    void addRecipe() {
        ...
        recipe.addStep(new RecipeStepEntity(null, 1, "Heat olive oil in a large pan, add diced onions, carrots, and celery, and cook until softened."));
        recipe.addStep(new RecipeStepEntity(null, 2, "Add minced garlic and ground beef, and cook until the beef is browned."));
        recipe.addStep(new RecipeStepEntity(null, 3, "Stir in tomato paste, canned tomatoes, and a splash of red wine, then season with salt, pepper, and Italian herbs."));
        recipe.addStep(new RecipeStepEntity(null, 4, "Reduce the heat and let the sauce simmer for 20–30 minutes, stirring occasionally."));
        recipe.addStep(new RecipeStepEntity(null, 5, "Serve the sauce over cooked spaghetti and top with freshly grated Parmesan."));
        ...
        ResponseEntity<RecipeModel> createResponse = restTemplate.postForEntity("http://localhost:" + port + "/api/recipe", recipe, RecipeModel.class);
        ...
        assertThat(createdRecipe.getSteps()).containsValue("Heat olive oil in a large pan, add diced onions, carrots, and celery, and cook until softened.");
        assertThat(createdRecipe.getIngredients()).isNotEmpty();
        assertThat(createdRecipe.getIngredients()).containsAll(recipe.getIngredients().stream().map(ingredient -> new IngredientModel(ingredient)).collect(Collectors.toList()));
        ...
        recipe.addStep(new RecipeStepEntity(null, 3, "Heat olive oil in a large pan, add diced onions, carrots, and celery, and cook until softened."));
        recipe.addStep(new RecipeStepEntity(null, 4, "Add minced garlic and ground beef, and cook until the beef is browned."));
        recipe.addStep(new RecipeStepEntity(null, 5, "Stir in tomato paste, canned tomatoes, and a splash of red wine, then season with salt, pepper, and Italian herbs."));
        recipe.addStep(new RecipeStepEntity(null, 6, "Reduce the heat and let the sauce simmer for 20–30 minutes, stirring occasionally."));
        recipe.addStep(new RecipeStepEntity(null, 7, "Serve the sauce over cooked spaghetti and top with freshly grated Parmesan."));

        recipe.addIngredient(getIngredient("celery", 1.0, "stalk"));

        ResponseEntity<RecipeModel> updateResponse = restTemplate.exchange("http://localhost:" + port + "/api/recipe/" + createdRecipe.getId(), HttpMethod.PUT, new HttpEntity<>(recipe), RecipeModel.class);
        ...
        assertThat(updatedRecipe.getIngredients()).containsAll(recipe.getIngredients().stream().map(ingredient -> new IngredientModel(ingredient)).collect(Collectors.toList()));
    ...
    void readRecipes() throws JsonProcessingException {
        ...
        for (SimpleRecipeEntity searchedRecipe : searchRecipes("", false).getContent()) {
            assertThat(searchedRecipe.getAuthor().getId()).isIn(1L, 2L, 3L, 4L, 21L); // the recipe's author must be one of the user's friends or the user themself
        }
        logout();
        ...
        assertThat(searchRecipes("", false).getContent()).contains(new SimpleRecipeEntity(successfulRead));
        logout();
    ...
    void readReviews() throws JsonProcessingException {
        ...
        for (ReviewEntity review : readReviewsByRecipe(1L, false).getContent()) {
            assertThat(review.getAuthor().getId()).isIn(1L, 2L, 3L, 4L, 5L, 21L); // the review's author must be one of the user's friends or the user themself
            assertThat(review.getRecipe().getId()).isEqualTo(1L);
            logger.info("review for recipe: 1 with value: " + (1 + review.getReview().ordinal()) + ", and content: " + review.getContent());
            totalReviews ++;
        ...
        for (ReviewEntity review : readReviewsByRecipe(5L, false).getContent()) {
            assertThat(review.getAuthor().getId()).isIn(1L, 2L, 3L, 5L, 6L, 7L, 20L, 21L); // the review's author must be one of the user's friends or the user themself
            logger.info("review for recipe: 5 with value: " + (1 + review.getReview().ordinal()) + ", and content: " + review.getContent());
            totalReviews ++;
            totalRating += (1 + review.getReview().ordinal());
        }
        logger.info("********************************");
        ...
        for (ReviewEntity review : readReviewsByRecipe(5L, false).getContent()) {
            assertThat(review.getAuthor().getId()).isIn(1L, 2L, 4L, 5L, 7L, 8L, 15L, 21L); // the review's author must be one of the user's friends or the user themself
            logger.info("review for recipe: 5 with value: " + (1 + review.getReview().ordinal()) + ", and content: " + review.getContent());
            totalReviews ++;
            totalRating += (1 + review.getReview().ordinal());
        }
    ...
    void deleteReview() throws JsonProcessingException {
        ...
        for (ReviewEntity reviewByUser : readReviewsByAuthor(21L, false).getContent()) {
            ...
            ResponseEntity<Void> deleteResponse = restTemplate
                .exchange("http://localhost:" + port + "/api/review/"+reviewtoDelete, HttpMethod.DELETE, null, Void.class);
        ...
        for (ReviewEntity review : readReviewsByRecipe(1L, false).getContent()) {
            assertThat(review.getAuthor().getId()).isIn(1L, 2L, 3L, 5L, 6L, 7L, 20L); // the review's author must be one of the user's friends or the user themself (but user 21's review has been deleted)
    ...
    void deleteUser() throws JsonProcessingException {
        ...
        ResponseEntity<Void> deleteResponse = restTemplate
            .exchange("http://localhost:" + port + "/api/user", HttpMethod.DELETE, null, Void.class);
        ...
        logger.info("******** user 1 recipe 1 *******");
        for (ReviewEntity review : readReviewsByRecipe(1L, false).getContent()) {
            assertThat(review.getAuthor().getId()).isIn(1L, 3L, 5L, 6L, 7L, 20L); // the review's author must be one of the user's friends or the user themself (but user 2's reviews have been deleted)
            logger.info("review for recipe: 1 with value: " + (1 + review.getReview().ordinal()) + ", and content: " + review.getContent());
            totalReviews ++;
            totalRating += (1 + review.getReview().ordinal());
        }
        logger.info("********************************");
        assertThat(readRecipe(1L, false).getRating()).isEqualTo(Math.round((totalRating / totalReviews) * 10) / 10.0);
        logout();
    }
    ...
    private synchronized void logout() {
        ...
        ResponseEntity<String> logoutResponse = restTemplate.postForEntity(logoutUrl, null, String.class);
        
        // Verify that the response status is OK, indicating a successful logout
        assertThat(logoutResponse.getStatusCode()).isEqualTo(HttpStatus.OK);
        
        // Clear any session cookies from the restTemplate to avoid reusing the session in future requests
        restTemplate.getRestTemplate().getInterceptors().removeIf(
            interceptor -> interceptor instanceof ClientHttpRequestInterceptor
        );
    ...
    private void request(Long friend, boolean expectFailure) {
        ResponseEntity<Void> response = restTemplate
            .postForEntity("http://localhost:" + port + "/api/friend/request/" + friend, null, Void.class);
        if (expectFailure) {
    ...
    private void block(Long user, boolean expectFailure) {
        ResponseEntity<Void> response = restTemplate
            .postForEntity("http://localhost:" + port + "/api/friend/block/" + user, null, Void.class);
        if (expectFailure) {
    ...
    private void accept(Long friend, boolean expectFailure) {
        ResponseEntity<Void> response = restTemplate
            .postForEntity("http://localhost:" + port + "/api/friend/accept/" + friend, null, Void.class);
        if (expectFailure) {
            assertThat(response.getStatusCode()).isNotEqualTo(HttpStatus.OK);
        } else {
            assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
    ...
    private RecipeIngredientEntity getIngredient(String query, Double quantity, String unit) {
        ...
        ResponseEntity<List<IngredientEntity>> response = restTemplate.exchange(
            url,
            HttpMethod.GET,
            null,
            new ParameterizedTypeReference<List<IngredientEntity>>() {}
        );
    ...
    private RecipeSkillEntity getSkill(String query) {
        ...
        ResponseEntity<List<SkillEntity>> response = restTemplate.exchange(
            url,
            HttpMethod.GET,
            null,
            new ParameterizedTypeReference<List<SkillEntity>>() {}
        );
    ...
    private RecipeEquipmentEntity getEquipment(String query) {
        ...
        // Send GET request to the ingredient search endpoint
        ResponseEntity<List<EquipmentEntity>> response = restTemplate.exchange(
            url,
            HttpMethod.GET,
            null,
            new ParameterizedTypeReference<List<EquipmentEntity>>() {}
        );
    ...
    private RecipeModel readRecipe(Long recipeId, boolean expectFailure) {
        ResponseEntity<RecipeModel> response = restTemplate
            .getForEntity("http://localhost:" + port + "/api/recipe/" + recipeId, RecipeModel.class);
        if (expectFailure) {
            assertThat(response.getStatusCode()).isNotEqualTo(HttpStatus.OK);
            return null;
        } else {
    ...
    private PagedModel<SimpleRecipeEntity> searchRecipes(String query, boolean expectFailure) throws JsonProcessingException {
        ...
        if (expectFailure) {
            ...
            JavaType pageType = mapper.getTypeFactory().constructParametricType(PagedModel.class, SimpleRecipeEntity.class);
            return mapper.readValue(response.getBody(), pageType);
        }
    }

    private ReviewEntity startCooking(Long recipe, boolean expectFailure) {
        ResponseEntity<ReviewEntity> response = restTemplate.postForEntity(
            "http://localhost:" + port + "/api/recipe/cook/" + recipe,
            null,
            ReviewEntity.class
        );
        if (expectFailure) {
    ...
    private void addReview(ReviewEntity review, boolean expectFailure) {
        ResponseEntity<ReviewEntity> response = restTemplate
            .postForEntity("http://localhost:" + port + "/api/review", review, ReviewEntity.class);
        if (expectFailure) {
            assertThat(response.getStatusCode()).isNotEqualTo(HttpStatus.CREATED);
        } else {
            assertThat(response.getStatusCode()).isEqualTo(HttpStatus.CREATED);
    ...
    private ReviewEntity updateReview(ReviewEntity review, boolean expectFailure) {
        ResponseEntity<ReviewEntity> response = restTemplate.exchange("http://localhost:" + port + "/api/review/" + review.getId(), HttpMethod.PUT, new HttpEntity<>(review), ReviewEntity.class);
        if (expectFailure) {
            assertThat(response.getStatusCode()).isNotEqualTo(HttpStatus.OK);
            return null;
        } else {
    ...
    private PagedModel<ReviewEntity> readReviews(boolean expectFailure) throws JsonProcessingException {
        ...
        if (expectFailure) {
            ...
            JavaType pageType = mapper.getTypeFactory().constructParametricType(PagedModel.class, ReviewEntity.class);
            return mapper.readValue(response.getBody(), pageType);
        }
    }

    private PagedModel<ReviewEntity> readReviewsByRecipe(Long recipeId, boolean expectFailure) throws JsonProcessingException {
    ...
    private PagedModel<ReviewEntity> readReviewsByAuthor(Long author, boolean expectFailure) throws JsonProcessingException {
    ...
    private ReviewEntity readReview(Long reviewId, boolean expectFailure) {
        ResponseEntity<ReviewEntity> response = restTemplate
            .getForEntity("http://localhost:" + port + "/api/review/" + reviewId, ReviewEntity.class);
        if (expectFailure) {
            assertThat(response.getStatusCode()).isNotEqualTo(HttpStatus.OK);
            return null;
        } else {
